  ...
  server.tool(
    ...
    "Connect to a Siglent SDG waveform generator over TCP. Returns device identification on success.",
    {
      host: z
        ...
        .optional()
        .describe(
          "IP address of the waveform generator (defaults to SIGLENT_SDG_IP or SIGLENT_IP env var)"
        ),
      port: z
        .number()
        .optional()
    ...
    async ({ host, port }) => {
      const targetHost =
        host || process.env.SIGLENT_SDG_IP || process.env.SIGLENT_IP;
      if (!targetHost) {
        return {
          content: [
            {
              type: "text" as const,
              text: "Error: No host specified. Provide a 'host' parameter or set the SIGLENT_SDG_IP environment variable.",
            },
          ],
          isError: true,
        };
